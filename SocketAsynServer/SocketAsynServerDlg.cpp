...
BEGIN_MESSAGE_MAP(CSocketAsynServerDlg, CDialog)
	...
	ON_BN_CLICKED(IDC_BUTTON_QUIT, &CSocketAsynServerDlg::OnBnClickedButton2)
END_MESSAGE_MAP()
...
void CSocketAsynServerDlg::OnReceive()
{
	...
	if (nReceived!=SOCKET_ERROR)
	{
		...
		********TX1人数识别模块，当前人数上传、存储**********
		***************************************************/
		if (pBuf[0] == 'p'&&pBuf[1] == 'n') {
			//donotdisplaypoint = 1;
			char *TX1_PersonNumber = new char[50];
			char *TX1ID = new char[10];
			char sqlstr[500] = {};
			int c = 0;
			int x = 2;
			while (pBuf[x] != '$')													
			...
			c = 0;
			while (pBuf[x] != '$')
				TX1_PersonNumber[c++] = pBuf[x++];
			TX1_PersonNumber[c] = '\0';
			//donotdisplaypoint = 1;
			strcpy(sqlstr, "INSERT INTO bodycount VALUES(");
			strcat(sqlstr,TX1ID);
			strcat(sqlstr, ", CURRENT_DATE,CURRENT_TIMESTAMP,");
			strcat(sqlstr, TX1_PersonNumber);
			strcat(sqlstr, ");");
			process_statement(conn, sqlstr);
			free(TX1_PersonNumber);
			free(TX1ID);
		}
